...
(function appendSupplements() {
  ...
  */
  ...
  /* ----------------- ArcGIS Pro Foundation (EAPF) ------------------------- */
  add('arcgis', [
    /* D1 Geographic Data Models — scenario-stem */
    { domain: 'D1', q: 'A GIS analyst receives a folder containing many shapefiles. She knows that shapefiles have several limits. Which of the following is the MOST significant limit she should warn her team about?',
      opts: ['Maximum 5 attribute fields', 'Single .shp file size limit of 2 GB and 10-character field names', 'Cannot store points', 'Cannot be edited in ArcGIS Pro'],
      answer: 1, exp: 'Each .shp / .dbf is capped at 2 GB and field names are limited to 10 characters. Both bite analysts in real projects (truncated names, file growth). The fix is moving to a file geodatabase.' },
    { domain: 'D1', q: 'A team is choosing between storing a regional rainfall surface as a polygon layer or as a raster. Which is the MORE APPROPRIATE choice and WHY?',
      opts: ['Polygon — rainfall has discrete category boundaries', 'Raster — continuous phenomena (rainfall, elevation, temperature) are stored as raster surfaces', 'Either works equally well', 'Polygon — vectors are always preferred'],
      answer: 1, exp: 'Continuous phenomena (no natural discrete boundaries) are best stored as a raster surface where every cell holds a value. DEMs, NDVI, temperature, rainfall = raster.' },
    { domain: 'D1', q: 'A new analyst opens an attribute table and asks why she cannot edit the OBJECTID field. What is the BEST response?',
      opts: ['She lacks write permissions on the file', 'OBJECTID is a system-managed unique identifier — never user-editable; joins, selections, and relationships depend on it', 'OBJECTID can be edited but only via Python', 'OBJECTID is the geometry column'],
      answer: 1, exp: 'OBJECTID is auto-generated by ArcGIS to give every row a stable unique key. Editing or removing it breaks joins, selections, and related-record links.' },
    { domain: 'D1', q: 'An analyst needs to model a road network — interconnected line features with traffic-flow analysis. Which dataset type is MOST appropriate?',
      opts: ['A single shapefile of roads', 'A network dataset (or a network layer in ArcGIS Pro/Online)', 'A raster of roads', 'A geocode service'],
      answer: 1, exp: 'Network datasets model topology + travel costs and support analyses like service area, closest facility, and route. Plain shapefiles lack the connectivity model.' },
    { domain: 'D1', q: 'A team is debating vector vs raster for a parcel-level land-use map. Each parcel has crisp legal boundaries. Which model is BEST and WHY?',
      opts: ['Raster — rasters are always faster', 'Vector (polygon) — parcels have discrete boundaries that vectors represent precisely', 'CAD format — only CAD preserves legal lines', 'Annotation layer — to label each parcel'],
      answer: 1, exp: 'Parcels = discrete, legally-defined boundaries → vector polygons. Rasters generalize boundaries to cell edges, which is the wrong representation for a parcel layer.' },

    /* D2 Coordinate Systems & Projections */
    { domain: 'D2', q: 'A user opens two layers in ArcGIS Pro: one in WGS 1984 geographic and one in a local State Plane projection. They appear to draw in roughly the same place, but spatial analysis between them returns warnings. What is the MOST likely cause and the FIRST step to fix it?',
      opts: ['The map is broken; restart Pro', 'The coordinate systems differ; project both to a common projected CRS that fits the area before analysis', 'Their geometries are corrupted', 'The display CRS is wrong but data are fine'],
      answer: 1, exp: 'Display can on-the-fly project, but distance/area analyses require both inputs in the same projected coordinate system. Project to a common CRS first.' },
    { domain: 'D2', q: 'A team mapping a small city for engineering must MINIMIZE distortion of distances and areas. Which projection family is the BEST fit?',
      opts: ['Mercator (a conformal world projection)', 'A local projected coordinate system (State Plane / UTM zone covering that city)', 'Robinson world projection', 'Web Mercator'],
      answer: 1, exp: 'For city-scale work with engineering tolerances, use State Plane (US) or the UTM zone covering the area. Both keep distance/area distortion very small over a small region.' },
    { domain: 'D2', q: 'A user reports that two layers from the same study area "don\'t line up" by a noticeable distance. Both are vector. Which is the MOST likely cause?',
      opts: ['One layer was created in a different datum (e.g., NAD27 vs NAD83) without a geographic transformation defined', 'GPS interference', 'ArcGIS Pro bug', 'Color mismatch'],
      answer: 0, exp: 'Datum mismatches (NAD27 vs NAD83, or any unprojected vs projected pair) commonly produce a constant shift of tens to hundreds of meters. Apply the correct geographic transformation when projecting.' },
    { domain: 'D2', q: 'Web Mercator is widely used for online basemaps. Which of the following is a TRUE limitation worth knowing?',
      opts: ['It is only legal in Europe', 'It severely distorts AREAS at high latitudes — Greenland appears as big as Africa', 'It cannot display roads', 'It only works in ArcGIS Online'],
      answer: 1, exp: 'Web Mercator is conformal (preserves angles) but heavily distorts areas at high latitudes. Use equal-area projections for thematic AREA-based maps.' },
    { domain: 'D2', q: 'Why is "on-the-fly projection" useful AND a common pitfall in ArcGIS Pro?',
      opts: ['It is only useful, never a pitfall', 'It lets layers in different CRS display together (useful), but ANALYSIS still requires both inputs share a common projected CRS — relying on display projection can produce incorrect distances and areas (pitfall)', 'It only works for rasters', 'It always crashes Pro'],
      answer: 1, exp: 'On-the-fly projection makes the map LOOK right even if CRS differ — but geoprocessing tools that compute distance / area / overlay need the inputs to actually share a projected CRS, or results will be wrong.' },

    /* D3 Cartography & Symbology */
    { domain: 'D3', q: 'An analyst is mapping county-level population. Counties vary in size from very small to huge. The map should show density, not just raw counts. Which BEST approach should she use?',
      opts: ['Use raw population values with a graduated-color choropleth — let viewers reason about size', 'Normalize population by county area to get density, then map density with a sequential color ramp', 'Use a categorical color scheme', 'Plot only the largest counties'],
      answer: 1, exp: 'Choropleths must be normalized when the underlying polygons are not uniform — otherwise the map mostly shows polygon size. Density (pop/area) reveals the spatial pattern.' },
    { domain: 'D3', q: 'A cartographer is choosing a color ramp for a map of unemployment rates by tract (continuous 0–25%). Which ramp is MOST APPROPRIATE?',
      opts: ['A categorical (qualitative) palette', 'A sequential single-hue ramp (light to dark)', 'A divergent ramp around a meaningful midpoint (only if there is one)', 'Random colors'],
      answer: 1, exp: 'Sequential single-hue ramps fit ordered continuous data with no natural midpoint. Divergent ramps are for data with a meaningful center (e.g., temperature anomaly around 0).' },
    { domain: 'D3', q: 'A team is preparing a map for print. Which of the following BEST describes "scale-dependent rendering" in ArcGIS Pro?',
      opts: ['Layers always display the same way at every scale', 'Layers can be configured to draw only within a defined scale range — e.g., individual building footprints only when zoomed in past 1:5,000', 'Pro automatically deletes data outside the visible area', 'Scale-dependence applies only to rasters'],
      answer: 1, exp: 'Scale ranges keep maps readable: detailed features only render at large scales; generalized labels appear at small scales. Defined per layer in the layer properties.' },
    { domain: 'D3', q: 'An accessibility review of a thematic map flags that color alone conveys the data class. Which TWO simple changes BEST improve accessibility? (Pick the BEST single answer that includes both.)',
      opts: ['Make the map larger only', 'Add patterns / hatching to differentiate classes AND use a colorblind-safe palette (e.g., from ColorBrewer)', 'Remove the legend', 'Use only red and green for contrast'],
      answer: 1, exp: 'Color alone fails color-vision-deficient users. Pair color with patterns OR pick a colorblind-safe palette. ColorBrewer publishes safe sequential, diverging, and qualitative ramps.' },
    { domain: 'D3', q: 'A map layout is missing a fundamental element required by cartographic best practice. Which is MOST commonly missing in beginner maps?',
      opts: ['Title', 'Scale bar AND north arrow', 'Legend', 'Frame border'],
      answer: 1, exp: 'Scale bar + north arrow + legend + title are the staple cartographic elements. Beginners frequently omit scale + orientation, especially the scale bar (which remains valid even if the user zooms).' },

    /* D4 Spatial Analysis & Geoprocessing */
    { domain: 'D4', q: 'An analyst needs to find all parcels WITHIN 500 ft of a proposed school. Which geoprocessing operation BEST answers this?',
      opts: ['Dissolve', 'Buffer the school by 500 ft, then Intersect with parcels (or Select By Location: parcels within 500 ft of school)', 'Clip', 'Spatial Join'],
      answer: 1, exp: 'Distance-based selection = buffer + overlay or directly Select By Location with the "within a distance of" relationship. Clip / dissolve don\'t answer "is it within 500 ft."' },
    { domain: 'D4', q: 'A team needs to count how many burglary points fall within each police-beat polygon. Which tool answers this DIRECTLY?',
      opts: ['Buffer', 'Spatial Join with COUNT statistic (or Summarize Within)', 'Dissolve', 'Project'],
      answer: 1, exp: 'Spatial Join (one-to-one, count) joins point counts to polygons. Summarize Within is the modern ArcGIS Pro equivalent. Both produce the per-polygon point count.' },
    { domain: 'D4', q: 'An analyst clips a roads layer to a study area boundary. The output\'s attribute schema lacks a field from the input. What is the MOST likely cause?',
      opts: ['Clip drops attributes randomly', 'Field-mapping / output schema configured to exclude that field, OR the input field was not flagged as required in the schema', 'Clip always drops the FID', 'A bug'],
      answer: 1, exp: 'Geoprocessing tools allow custom output schemas via field maps. If a field appears missing, check field mapping. Otherwise Clip preserves the input schema.' },
    { domain: 'D4', q: 'A team needs a slope raster from a DEM. Which tool fits — and what should they CHECK FIRST about the inputs?',
      opts: ['Slope tool; check that the DEM is in a projected CRS (not geographic lat/lon) so output is in degrees or percent correctly', 'Aspect tool; nothing matters', 'Resample tool; check the screen brightness', 'Clip tool; no input check needed'],
      answer: 0, exp: 'Slope expects horizontal units that match vertical (meters/meters or feet/feet). A geographic CRS in decimal degrees produces nonsensical slopes — project to UTM or State Plane first.' },
    { domain: 'D4', q: 'A spatial analyst observes that a kernel-density surface looks "fine" with one bandwidth but very different with another. Which BEST captures the cartographic implication?',
      opts: ['Density surfaces are objective truths', 'Choice of bandwidth (search radius) materially changes the surface; document the value and justify it for reproducibility', 'Bandwidth is fixed by ArcGIS Pro', 'Smaller is always better'],
      answer: 1, exp: 'Kernel-density results depend strongly on the bandwidth chosen. Document the value and the rationale (e.g., "1 km — to match average pedestrian access in this study"). Sensitivity-test results.' },

    /* D5 ArcGIS Online & Web GIS */
    { domain: 'D5', q: 'A small team wants to publish an interactive web map showing 30 layers, with editing for one of the layers. Which item type in ArcGIS Online is MOST APPROPRIATE for the EDITABLE layer?',
      opts: ['Tile layer', 'Feature layer (hosted)', 'Imagery layer', 'CSV file'],
      answer: 1, exp: 'Hosted feature layers support reads AND edits with attribute / geometry capabilities. Tile/imagery layers are read-only cache. CSV is a static file.' },
    { domain: 'D5', q: 'A user shares a web map "to organization" but external partners say they cannot open it. What is the MOST likely cause?',
      opts: ['ArcGIS Online is broken', 'Sharing scope is set to "organization" only — change to "Everyone (public)" or share with a specific group that includes the partners', 'The map has too many layers', 'The map needs republication'],
      answer: 1, exp: 'Sharing scope (Private / Org / Group / Public) is the most common access issue. Match the audience to the sharing level; group sharing is best for limited audiences.' },
    { domain: 'D5', q: 'An analyst wants users to filter web-map data by category from a dropdown without writing code. Which AGOL feature BEST fits?',
      opts: ['ArcGIS Pro print layout', 'Configure pop-ups in the web map; for richer UI, build a Web AppBuilder / Experience Builder / Dashboards app with a filter widget', 'Excel macros', 'A static PDF export'],
      answer: 1, exp: 'No-code apps (Experience Builder, Dashboards, Instant Apps) include filter widgets. Pop-ups handle per-feature detail but not user-driven filtering.' },
    { domain: 'D5', q: 'A team\'s tile layer (basemap-like) takes minutes to load on first visit. Which BEST describes WHY tile layers are usually fast — and what may be happening here?',
      opts: ['Tile layers are slow by design', 'Tile layers cache pre-rendered tiles for speed; first visit may be slow if tiles for that area / zoom are being generated on-demand or have not been cached yet', 'Tile layers do not cache', 'The map is too small'],
      answer: 1, exp: 'Tile layers are cached pre-rendered images for fast delivery. If a zoom level / area isn\'t cached, the system may build tiles on-demand → first hit is slow; subsequent loads from cache are fast.' },
    { domain: 'D5', q: 'A team configures a Dashboard with several indicators and a list. Which AGOL concept lets the dashboard show ONLY records that match a filter selection across all elements?',
      opts: ['Pop-up scripting', 'Element-to-element interactions (selection actions / filters) so a selection in one element drives the others', 'Hard-coded URL parameters only', 'Layer visibility toggle'],
      answer: 1, exp: 'Dashboard elements interact via configured actions (selection / filter). Clicking a category in one indicator can filter the list and other widgets. No code required.' },

    /* D6 Geodatabases, Metadata & Sharing */
    { domain: 'D6', q: 'A multi-user editing scenario requires concurrent edits with conflict handling. Which geodatabase architecture is MOST APPROPRIATE?',
      opts: ['File geodatabase on a shared drive', 'Personal geodatabase (Access .mdb)', 'Enterprise geodatabase (multi-user, versioned editing — SQL Server / Oracle / PostgreSQL)', 'Shapefile on a shared drive'],
      answer: 2, exp: 'Enterprise geodatabases support multi-user concurrent editing, versioning, and conflict reconciliation. File GDB is single-writer / multi-reader; personal GDB is legacy single-user.' },
    { domain: 'D6', q: 'A team wants other agencies to discover and reuse their data. Which BEST captures the role of metadata?',
      opts: ['Metadata is optional and rarely used', 'Metadata describes the data — purpose, contact, lineage, projection, accuracy, constraints — so others can evaluate fitness for use and find the dataset in catalogs', 'Metadata replaces the actual data', 'Metadata is only useful for raster'],
      answer: 1, exp: 'Metadata standards (ISO 19115, FGDC) describe the data\'s purpose, lineage, quality, and constraints. Without it, others cannot tell if the dataset fits their use case or find it via search.' },
    { domain: 'D6', q: 'An analyst is sharing a project package. What is the difference between a Map Package (.mpkx) and a Project Package (.ppkx) in ArcGIS Pro?',
      opts: ['They are identical', '.mpkx contains a single map (layers + data); .ppkx contains an ENTIRE project (multiple maps, toolboxes, layouts, tasks, data)', 'Only .ppkx works on Mac', 'Both contain only links to data'],
      answer: 1, exp: 'Map packages bundle a single map and its data. Project packages bundle the whole project — maps, layouts, toolboxes, tasks. Choose based on scope.' },
    { domain: 'D6', q: 'A team needs a versioned, multi-user editing environment with branch versioning. Which BEST describes BRANCH versioning vs TRADITIONAL versioning?',
      opts: ['They are the same thing', 'Branch versioning uses services + named branches that can be merged; traditional versioning uses database states managed inside the geodatabase. Branch is the modern Pro / Enterprise approach for many workflows.', 'Branch versioning works only on shapefiles', 'Traditional versioning is faster'],
      answer: 1, exp: 'Branch versioning (Pro/Enterprise) uses feature services and branches that can be merged like in Git, simplifying collaboration. Traditional versioning uses database state-based versions. Use the model your enterprise supports.' },
    { domain: 'D6', q: 'A user wants to share a layer\'s STYLE (symbology, labels) without sharing the underlying data. Which file format is APPROPRIATE?',
      opts: ['A .ppkx project package', 'A .lyrx layer file (style only; references data location)', 'A shapefile (.shp)', 'A geodatabase'],
      answer: 1, exp: 'Layer files (.lyrx) record symbology, labels, definition queries, and a reference to the data source — they do not bundle the data itself. Use for style sharing.' },
    { domain: 'D6', q: 'A user copies a shapefile by selecting only the .shp file. The destination ArcGIS Pro session reports the dataset is invalid. What is the MOST likely cause?',
      opts: ['ArcGIS Pro bug', 'A shapefile is actually a SET of files — .shp .shx .dbf at minimum, plus .prj / .cpg / .sbx — copying only .shp loses geometry index and attributes', 'The .shp is corrupted', 'Shapefiles cannot be moved'],
      answer: 1, exp: 'Shapefile = sidecar files. Always copy the entire group (the easy way is to use ArcGIS Catalog / Pro\'s Catalog pane which treats the bundle as one item).' }
  ]);
